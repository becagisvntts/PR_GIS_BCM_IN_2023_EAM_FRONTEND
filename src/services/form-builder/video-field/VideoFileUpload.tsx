...
import VideoFieldConfig from './VideoFieldConfig'
...
import FullscreenVideoDialog from 'src/views/components/common/FullscreenVideoDialog'
...
export default function VideoFileUpload({
  ...
  const [openDialogViewVideo, setOpenDialogViewVideo] = useState(false)
  const [openingVideoPath, setOpeningVideoPath] = useState<string>('')
  ...
  const handleFileChange = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (files) {
      ...
      for (let i = 0; i < files.length; i++) {
        const file = files[i]
        const validated = VideoFieldConfig.validateImage(file)
        if (validated === true) {
    ...
    <Box>
      {openingVideoPath && (
        <FullscreenVideoDialog
          imagePath={openingVideoPath}
          onClose={() => setOpenDialogViewVideo(false)}
          openDialog={openDialogViewVideo}
        />
      )}
      <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 5 }}>
        ...
        {uploading ? (
          ...
          >
            {LocalizationService.translate('form_pick_video')}
            <input
              type='file'
              hidden
              multiple={allowMultiple}
              accept={VideoFieldConfig.videoAllowExtensions.join(', ')}
        ...
        <Box sx={{ display: 'flex', justifyContent: 'start', alignItems: 'start', flexFlow: 'wrap', flex: 'auto' }}>
          {selectedFiles.map(el => (
            ...
            >
              <Button
                onClick={() => {
                  setOpeningVideoPath(`${serverUrl}${el.path}`)
                  setOpenDialogViewVideo(true)
                }}
                sx={{ p: 0 }}
              >
                <video
